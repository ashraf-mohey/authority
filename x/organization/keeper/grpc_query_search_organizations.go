...
func (k Keeper) SearchOrganizations(goCtx context.Context, req *types.QuerySearchOrganizationsRequest) (*types.QuerySearchOrganizationsResponse, error) {
	if req == nil {
	...
	pageRes, err := query.Paginate(organizationsStore, req.Pagination, func(key []byte, value []byte) error {
		...
		if ContainsI(organization.Name, req.NameQuery) {
	...
	// Return a struct containing a list of organizations records and pagination info
	return &types.QuerySearchOrganizationsResponse{Organizations: organizations, Pagination: pageRes}, nil
}
